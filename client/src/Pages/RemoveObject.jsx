...
const RemoveObject = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (!trimmed) {
        toast.error("Please provide object name to remove");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 items-start flex flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.files[0])} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600' type="file" accept='image/*' required />

        <p className='mt-6 text-sm font-medium'> Describe Object name to Remove</p>

        <textarea onChange={(e) => setObject(e.target.value)} value={object} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' type="text" placeholder='e.g., watch, phone, Only Single Object name' required />
