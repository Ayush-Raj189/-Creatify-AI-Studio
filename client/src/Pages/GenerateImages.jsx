...
const GenerateImages = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const prompt = `Generate an image in ${selectedStyle} on the topic: ${input}.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 items-start flex flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' type="text" placeholder='Describe What You want to see in Image...' required />
        ...
        <div className='my-6 flex items-center gap-2 '>
          <label className='relative curor-pointer'>
